...
if (!empty($_POST["cambiarContraseña"]) && !empty($_POST["confirmarContraseña"])) {
    ...
    if($confirmarContraseñaHash === $contraseñaActual && $cambiarContraseñaHash === $contraseñaActual) { // Validar si la nueva contraseña es igual a la contraseña anterior
        header("location: login.html?login=success&cod=$codigo&login=fail&error=same_passwords"); 
    } 
